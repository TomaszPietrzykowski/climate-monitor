...
exports.decimalToMonth = (inputArray) => {
  const outputArray = inputArray.map((string) => {
    const year = string.split(".")[0]
    const decimal = parseInt(string.split(".")[1]) * 100
    const month = Math.ceil(decimal / 833)
    const output = `${year}-${month === 0 ? 1 : month}`
    return output
  })
  return outputArray
}
